...
void SUBCLU::run() {
    ...
    for (int dim=0; dim<dimensionality; dim++) {
        ...
        vector<Cluster> clusters = runDBSCAN(currSubspace);
    ...
    for(auto it = Clusterings.rbegin(); it!=Clusterings.rend(); it++) {
        ...
        if(subspace.getDimensionality() < minDim) continue;
...
vector<Subspace> SUBCLU::generateSubspaceCandidates(vector<Subspace> &subspaces) {
    if(subspaces.empty()) return vector<Subspace>();
...
bool SUBCLU::checkLower(Subspace& candidate, vector<Subspace> &subspaces) {
    ...
    for(int dim = 0; dim<dimensions.size(); dim++) {
        if(dimensions[dim] == 1) {
            ...
            temp[dim] == 0;
            if(!contains(subspaces, temp)) return false;
        }
    }
    return true;
}
...
bool SUBCLU::contains(vector<Subspace> &subspaces, vector<int> temp) {
    for(Subspace subspace: subspaces) {
        ...
        for(int i = 0; i<comp.size(); i++) {
            if(i>=temp.size() && comp[i]==1) flag = 0;
            else if(comp[i] != temp[i] ) flag = 0;
        }
        if(flag) return true;
    }
    return false;
...
Subspace SUBCLU::besttSubspace(vector<Subspace>& subspaces, Subspace& candidate) {
    ...
    for(Subspace subspace: subspaces) {
        if(subspace.isSubspace(candidate)) {
            int sum=0;
            for(Cluster cluster: Clusterings[subspace]) sum+=cluster.size(); 
            if(sum < min) {
                min = sum;
                theBest = subspace;
            }
